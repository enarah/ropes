...
export async function DashboardContent({
  ...
  return (
    <div className="space-y-6">
      ...
      {moduleSlug === "reports" &&
      organisationHasCapability(capabilityKeys, "reporting.appb") ? (
        <ReportingAppbLink organisationSlug={selectedOrganisation.slug} />
      ) : null}
...
function ReportingAppbLink({ organisationSlug }: { organisationSlug: string }) {
  return (
    <section className="rounded-md border border-earth-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-xl font-semibold text-charcoal-950">
            APP&B reporting
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-charcoal-600">
            Capability-gated planning surface for future grant-scoped NIAA and
            DCCEEW APP&B template mapping. Workbook generation is not
            implemented yet.
          </p>
        </div>
        <Link
          className="inline-flex w-fit rounded-md bg-ochre-600 px-4 py-2 text-sm font-semibold text-white"
          href={`/reports/appb?org=${organisationSlug}`}
        >
          Open APP&B
        </Link>
      </div>
